...
class Objectv(object):
	...
	def __init__(self, input_file):
		...
		with open(input_file, "r") as object_file:
			...
			for i in range(self.facenum):
				...
				for j in range(int(temp_line[0])):  # temp_line[0] contains number of vert in face
	...
	def paint(self, width, height):
		...
		if self.persp:
			for i in range(self.vertnum):
				self.view[i][0] = (orgx * self.view[i][0] / self.view[i][2]) + orgx
				self.view[i][1] = (orgy * self.view[i][1] / self.view[i][2]) + orgy
		...
		self.frames += 1
		canvas.draw_text('Object: ' + self.name + '   Frames: ' + str(self.frames)+ '   Angle: ' + str(self.yinc), 10, 10)
		canvas.end_updates()
